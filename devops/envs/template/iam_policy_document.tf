...
# These policies are only needed in prod.
# These are admin operated actions (starting/stopping glue/step functions)
# They are attached to the bastion instance role, so admin must have access to it
# (via SSM)
