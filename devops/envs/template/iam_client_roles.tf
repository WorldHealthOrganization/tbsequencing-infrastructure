locals {
  policies = [
    {
      name        = "rds_access"
      description = "Policy to allow connection to RDS via IAM authentication"
    ...
    {
      name        = "ncbi_secret_access"
      description = "Reading the NCBI email and api keys"
  ...
  policy_mapping = {
    ...
    ncbi_secret_access = {
      role   = module.roles.role_name["fargate-task"]
      policy = module.policies.policy_arn["ncbi_secret_access"]
    }
  ...
  roles = [
    ...
    },
    {
      ...
      custom_trust_policy     = data.aws_iam_policy_document.fargate_role.json
    },
    {
      name                    = "fargate-task"
      instance_profile_enable = null
