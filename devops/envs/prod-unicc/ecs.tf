module "ecs" {
  source       = "git::git@bitbucket.org:awsopda/who-seq-treat-tbkb-terraform-modules.git//ecs_cluster?ref=ecs-v1.0"
...
module "ecs_tasks" {
  source           = "git::git@bitbucket.org:awsopda/who-seq-treat-tbkb-terraform-modules.git//ecs_tasks?ref=ecs_tasks-v1.1"
  environment      = var.environment
...
locals {
  ...
  task_definitions = {
    "${local.prefix}-backend-migrations" = {
      ...
      task_role_arn                = module.roles.role_arn["fargate"]
      execution_role_arn           = module.roles.role_arn["fargate-execution-role"]
      cpu                          = 1024
      memory                       = 2048
  ...
  backend_secret_environments = [
    {
      "name" : "SECRET_KEY"
      "valueFROM" : "${resource.aws_secretsmanager_secret.django.arn}:SECRET_KEY::"
    },
    {
      "name" : "ADFS_TENANT_ID"
